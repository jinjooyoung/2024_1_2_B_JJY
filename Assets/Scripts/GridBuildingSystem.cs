...
public class GridBuildingSystem : MonoBehaviour
{
    ...
    private Camera firstPersonCamera;               // 플레이어의 1인칭 카메라
    ...
    // 그리드 셀에 건물을 배치하는 메서드
    private void RemoveBuild(Vector3Int gridPosition)
    {
        GridCell cell = cells[gridPosition.x, gridPosition.z];          // 그리드 셀을 통해서 해당 타일 데이터에 접근
        ...
        if (cell.IsOccupied)                                            // 셀에 건물이 있을 경우
        {
            ...
            cell.IsOccupied = false;                                    // 셀에 건물이 없다고 표시
            cell.Building = null;                                       // 건물 오브젝트 데이터를 삭제
        }
    }
    ...
    private void CreateGrid()
    {
        ...
        cells = new GridCell[width, height];                            // 그리드 가로 세로 칸 수 설정
        Vector3 gridCenter = playerController.transform.position;       // 플레이어 중심으로 그리드 생성 (원하는 위치로 변경 가능)
        gridCenter.y = 0;
        transform.position = gridCenter - new Vector3(width * cellSize / 2.0f, 0, height * cellSize / 2);       // 그리드 중심으로 이동시킨다
    ...
    private Vector3 GetLookPosition()
    {
        if (playerController.isFirstPerson)     // 플레이어의 1인칭 모드
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit hitInfo, maxBuildDistance))     // 설정한 ray에 물체가 있는 경우
            {
        ...
        else
        {
            ...
            Vector3 rayOrigin = characterPosition + Vector3.up * 1.5f + characterForward * 0.5f;    // 레이를 시작할 위치 설정
            ...
            if (Physics.Raycast(ray, out RaycastHit hitInfo, maxBuildDistance))     // 설정한 ray에 물체가 있는 경우
            {
                Debug.DrawRay(ray.origin, ray.direction * hitInfo.distance, Color.blue);    // Scene 창에서 파란색 선으로 ray를 보여준다.
                return hitInfo.point;                                                       // raycast 된 물체 정보를 리턴한다.
            }
            else
            {
